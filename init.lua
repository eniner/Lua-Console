...
local function buildSafeBase()
    ...
    -- Lua 5.1 compatibility for load/loadstring/setfenv/getfenv when available.
    if _G.load then base.load = _G.load end
    if _G.loadstring then base.loadstring = _G.loadstring end
    if _G.setfenv then base.setfenv = _G.setfenv end
    if _G.getfenv then base.getfenv = _G.getfenv end
...
local function makeEnv()
    ...
    local env = {
        ...
        cmd = function(command) return mq.cmd(command) end,
        cmdf = function(fmt, ...) return mq.cmdf(fmt, ...) end,
        delay = function(ms, condition) return mq.delay(ms, condition) end,
        echo = function(msg) return mq.cmdf('/echo %s', tostring(msg)) end,
...
local function loadPersistentState()
    ...
    if type(decoded.windowState) == 'table' then
        state.windowState = {
            x = tonumber(decoded.windowState.x),
            y = tonumber(decoded.windowState.y),
            w = tonumber(decoded.windowState.w) or state.windowState.w,
            h = tonumber(decoded.windowState.h) or state.windowState.h,
            collapsed = decoded.windowState.collapsed == true,
        }
    end
...
loadSettings = function()
    ...
    if type(data.windowState) == 'table' then
        local ws = data.windowState
        state.windowState = {
            x = tonumber(ws.x),
            y = tonumber(ws.y),
            w = tonumber(ws.w) or state.windowState.w,
            h = tonumber(ws.h) or state.windowState.h,
            collapsed = ws.collapsed == true,
        }
    end
...
local function printHelp()
    ...
    logInfo('/lua <code> - evaluate chunk (supports multiline continuation)')
...
local function drawWatchesPanel()
    ...
    ImGui.SameLine()
    if ImGui.Button('Add Watch', 90, 0) then
        addWatch(state.watchInput, state.watchLabelInput)
        state.watchInput = ''
        state.watchLabelInput = ''
    end
    ImGui.SameLine()
    if ImGui.Button('List', 52, 0) then listWatches() end
    ImGui.SameLine()
    if ImGui.Button('Clear##Watches', 80, 0) then
        state.watches = {}
        saveWatchesPersistent()
    ...
    if ImGui.BeginTable('##WatchTable', 6, bit32.bor(ImGuiTableFlags.RowBg, ImGuiTableFlags.Borders, ImGuiTableFlags.Resizable)) then
        ...
        for _, w in ipairs(state.watches) do
            ...
            if ImGui.SmallButton(('Del##Watch%d'):format(w.id)) then
                removeWatch(w.id)
                break
            end
        end
...
local function drawPlotsPanel()
    ...
    ImGui.SameLine()
    if ImGui.Button('Add Plot', 75, 0) then
        addPlot(state.watchInput or '', state.watchLabelInput or '', 300)
    end
    ImGui.SameLine()
    if ImGui.Button('Clear##Plots', 90, 0) then
        state.plots = {}
    end
    ...
    for _, p in ipairs(state.plots) do
        ...
        if ImGui.SmallButton(('Remove##Plot' .. p.id)) then
            removePlot(p.id)
            break
        end
    end
...
local function drawTriggersPanel()
    ...
    ImGui.SameLine()
    if ImGui.Button('Add Trigger', 90, 0) then
    ...
    ImGui.SameLine()
    if ImGui.Button('List##Trig', 60, 0) then listTriggers() end
    ImGui.SameLine()
    if ImGui.Button('Clear##Trig', 80, 0) then
        state.triggers = {}
    ...
    if ImGui.BeginTable('##TriggerTable', 8, bit32.bor(ImGuiTableFlags.RowBg, ImGuiTableFlags.Borders, ImGuiTableFlags.Resizable)) then
        ...
        for _, t in ipairs(state.triggers) do
            ...
            if ImGui.SmallButton(('Del##Trig%d'):format(t.id)) then
                removeTrigger(t.id)
                break
            end
        end
...
local function drawSnippetsPanel()
    ...
    if ImGui.Button('Save Snippet', 100, 0) then
    ...
    ImGui.SameLine()
    if ImGui.Button('Import##Snip', 80, 0) then importSnippets(SNIPPETS_FILE) end
    ImGui.SameLine()
    if ImGui.Button('Export##Snip', 80, 0) then exportSnippets(SNIPPETS_FILE) end
    ...
    if ImGui.BeginTable('##SnippetTable', 4, bit32.bor(ImGuiTableFlags.RowBg, ImGuiTableFlags.Borders, ImGuiTableFlags.Resizable)) then
        ...
        for i, s in ipairs(state.snippets) do
            ...
            if ImGui.SmallButton(('Run##Snip%d'):format(i)) then runSnippet(i) end
            ImGui.SameLine()
            if ImGui.SmallButton(('Load##Snip%d'):format(i)) then
                state.snippetNameInput = s.name
                state.snippetCodeInput = s.code
            end
            ImGui.SameLine()
            if ImGui.SmallButton(('Del##Snip%d'):format(i)) then
                removeSnippet(i)
                break
            end
        end
...
local function drawEventTesterPanel()
    ...
    if ImGui.Button('Run Event Test', 110, 0) then runEventTest() end
    ImGui.SameLine()
...
local function drawBenchPanel()
    ...
    if ImGui.Button('Run Benchmark', 110, 0) then
        runBenchmark(state.bench.code, state.bench.iterations)
    end
    ImGui.SameLine()
    if ImGui.Button('Use Input Buffer', 120, 0) then
...
local function drawSharePanel()
    ...
    if ImGui.Button('Export Share Bundle', 140, 0) then
        exportShareBundle(SHARE_FILE)
    end
    ImGui.SameLine()
    if ImGui.Button('Import Share Bundle', 140, 0) then
        importShareBundle(SHARE_FILE)
    end
    ImGui.SameLine()
    if ImGui.Button('Export Log', 100, 0) then
...
local function drawInputArea()
    ...
    if ImGui.Button('Eval', 80, 0) then
        evalRequested = true
    end

    ImGui.SameLine()
    if ImGui.Button('Clear Input', 90, 0) then
        state.inputBuffer = ''
    end
    ...
    ImGui.SameLine()
    if ImGui.Button('Clear Output', 95, 0) then
        state.logs = {}
        logInfo('Console log cleared.')
    end

    ImGui.SameLine()
    if ImGui.Button('Copy output', 100, 0) then
        local lines = {}
        for _, row in ipairs(state.logs) do
    ...
    ImGui.SameLine()
    if ImGui.Button('Load', 60, 0) then
        loadSession()
    end
    ...
    ImGui.SameLine()
    if ImGui.Button('Export Log', 88, 0) then
        exportLog(LOG_EXPORT_FILE)
    end
...
logInfo('REPL env shortcuts: T, Me, Target, Group, Spawn, Zone, mq, cmd, delay, inspect/dump/pp')
